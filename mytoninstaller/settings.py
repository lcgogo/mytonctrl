...
def migrate_network_config():
	global_config = "/usr/bin/ton/global.config.json"
	network_config = "/var/ton-work/global.config.json"
	args = ["cp", global_config, network_config]
	subprocess.run(args)

	# TODO
	# добавить изменение настроек ноды - загружать конфиг сети из новго места (нужно отойти от /usr/bin/ton/global.config.json)
#end define
